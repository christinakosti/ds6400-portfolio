...
The authors explain that choosing classifier parameters is part of the model training process. When several parameter values are tested using cross-validation, the value with the lowest CV error is selected. Because this minimum is chosen from several CV estimates, it may be too optimistic and underestimate the true prediction error. The authors call this parameter selection bias.

For example, for the Shrunken Centroids classifier, the authors selected the value of the tuning parameter Δ that minimized the 10-fold cross-validation error:

$$
\Delta^* = \arg\min_{\Delta} CV(\Delta)
$$

For the SVM classifier, the tuning parameters C and γ were selected to minimize the Leave-One-Out Cross-Validation (LOOCV) error:
...
The authors propose that classifier parameter tuning should be treated as part of the model training procedure and therefore repeated inside the cross-validation process.
...
The authors used a simulation study so they could compare the estimated prediction error with the true error.

For each simulation, they generated at least 1,000 datasets with:
...
For the SVM, the authors tuned two parameters, C and γ. They used Leave-One-Out Cross-Validation (LOOCV) to calculate the prediction error for different combinations of these parameters and selected the combination with the minimum LOOCV error.
...
The authors then evaluated a nested cross-validation procedure in which the parameter-tuning step was repeated inside an outer CV loop.

For the Shrunken Centroids classifier, one sample was left out from the 40-sample dataset. The remaining 39 samples were used to select the optimal value of \$\\Delta\$ using 10-fold CV. The classifier was then trained on those 39 samples using the selected \$\\Delta\$ and used to predict the class of the one sample that had been left out.
...
The authors conclude that nested CV provides an almost unbiased estimate of the true error of an optimized classifier.
...
I will use a k-nearest neighbors (kNN) classifier and treat the number of neighbors, k, as the tuning parameter. I will:

1.  Generate a small simulated training dataset.
2.  Use cross-validation to compare several values of k and select the value with the lowest CV error.
3.  Record this minimum CV error.
4.  Use nested cross-validation, where k is selected in the inner CV loop and prediction error is measured in the outer CV loop.
5.  Train the optimized classifier and evaluate it on a large independently simulated test dataset to approximate the true prediction error.
6.  Repeat the simulation many times and compare the three error estimates.
